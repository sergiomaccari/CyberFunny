...
namespace CyberMetro {
	...
	namespace Entidades {
		...
		{
			const float Jogador::PROJETIL_VELOCIDADE_JOGADOR = 4.0f;
			...
			void Jogador::executar()
			{
				if (this->getAtivo())
				{
					...
					if (this->y > MUNDO_Y_MAX + 200)//evitar que caia pra sempre
					{
						this->setAtivo(false);
					}
				}
			...
			json Jogador::salvarDataBuffer() const
			{
				json j = Personagem::salvarDataBuffer();
				j["tipo"] = "Jogador";
				j["pontos"] = this->pontos;
				j["playerNum"] = this->playerNum;
				j["ultimaDirecaoX"] = this->ultimaDirecaoX;
				return j;
			}
