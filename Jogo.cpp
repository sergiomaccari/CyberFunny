...
namespace CyberMetro {
        ...
        while (estadoAtual != EstadoJogo::Sair && gg.getJanelaOpen())
        {
            ...
            if (estadoAtual == EstadoJogo::Jogando || estadoAtual == EstadoJogo::FaseDois)
            {
                if (faseAtual)
                {
					...
					{//variaveis que determinam pontuacao e salvamento
    ...
    void Jogo::salvarJogo(const std::string& arquivo)
    {
        ...
        j_save["jogador1"] = pJogador1->salvarDataBuffer();
        j_save["jogador2"] = pJogador2->salvarDataBuffer();

        j_save["inimigos"] = json::array();
        j_save["obstaculos"] = json::array();
        j_save["projeteis"] = json::array();

        auto* inimigos = faseAtual->getListaInimigos()->getPrimeiro();
        while (inimigos) {
            if (inimigos->getInfo()->getAtivo()) {
                j_save["inimigos"].push_back(inimigos->getInfo()->salvarDataBuffer());

                Entidades::Personagens::Robo_CEO* ceo = dynamic_cast<Entidades::Personagens::Robo_CEO*>(inimigos->getInfo());
                if (ceo) {
                    auto* projCeo = ceo->getProjeteis()->getPrimeiro();
                    while (projCeo) {
                        if (projCeo->getInfo()->getAtivo()) {
                            j_save["projeteis"].push_back(projCeo->getInfo()->salvarDataBuffer());
                        }
        ...
        auto* obstaculos = faseAtual->getListaObstaculos()->getPrimeiro();
        while (obstaculos) {
            if (obstaculos->getInfo()->getAtivo()) {
                j_save["obstaculos"].push_back(obstaculos->getInfo()->salvarDataBuffer());
            }
            obstaculos = obstaculos->getProx();
        }

        auto* projP1 = pJogador1->getProjeteis()->getPrimeiro();
        while (projP1) {
            if (projP1->getInfo()->getAtivo()) {
                j_save["projeteis"].push_back(projP1->getInfo()->salvarDataBuffer());
            }
            projP1 = projP1->getProx();
        }

        if (pJogador2->getAtivo()) {
            auto* projP2 = pJogador2->getProjeteis()->getPrimeiro();
            while (projP2) {
                if (projP2->getInfo()->getAtivo()) {
                    j_save["projeteis"].push_back(projP2->getInfo()->salvarDataBuffer());
                }
                projP2 = projP2->getProx();
            }
        }
        ...
        if (o.is_open()) {
            ...
            o.close();
            std::cout << "Jogo salvo em: " << arquivo << std::endl;
        }
        else {
            std::cerr << "Erro: Nao foi possivel salvar o jogo em: " << arquivo << std::endl;
        }
    }
    ...
    void Jogo::carregarJogo(const std::string& arquivo)
    {
        ...
        if (!i.is_open()) {
            std::cerr << "Erro: Nao foi possivel carregar o save: " << arquivo << std::endl;
            estadoAtual = EstadoJogo::EntrandoNome;
            return;
        }
        ...
        std::cout << "Jogo carregado de: " << arquivo << std::endl;
    }
